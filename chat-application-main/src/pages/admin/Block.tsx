...
function Block() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      await axios.put(`users/${idSelected.current}`, {
        banned: {
          bannedUntil:
            checked === 2
              ? Date.now() + +daysRef.current!.value * 1000 * 60 * 60 * 24
              : null,
        },
      });
  ...
  const handleUnblock = async (id : any) => {
    ...
    try {
      await axios.put(`users/${id}`, { banned: null });
      setMessage("تم فك الحظر عن العضو");
      setRender((prev) =>!prev);
    } catch (error) {
      console.log(error);
      setMessage("لم يتم فك الحظر عن العضو");
    }
  };
  return (
    <div className="flex flex-col gap-y-4 py-4 px-2">
      <div className="relative">
        ...
        <ul className="absolute top-[%100] left-0 right-0 bg-white z-10">
          {activeSearchNameRef.current &&
            searchName &&
            users
              .filter((user: any) => user.name.includes(searchName))
              .slice(0, 5)
              .map((user: any) => (
                <li
                  key={user._id}
                  className="border-[1px] p-1 flex gap-2 items-center cursor-pointer"
                  onClick={() => {
                ...
                >
                  <img
                    className="w-8"
                    src={import.meta.env.VITE_API_BASE_URL + user.img}
                    alt=""
                  />
                  <div>
                    <p>{user.name}</p>
        ...
        <Table variant="simple">
          ...
          <Tbody>
            {users
              ...
              .map((user: any) => (
                <Tr key={user._id} className="bg-green-50">
                  <Td border={"1px solid gray"}>{user.name}</Td>
                  <Td border={"1px solid gray"}>
                    {user.banned.bannedUntil ? "مؤقت" : "دائم"}
                  </Td>
                  <Td
                    border={"1px solid gray"}
                    className="!text-center "
                  >
                    {user.banned.bannedUntil
                      ? formateDate(user.banned.bannedUntil)
                      : "🚫"}
                  </Td>
                  <Td border={"1px solid gray"}>
                    <Button
