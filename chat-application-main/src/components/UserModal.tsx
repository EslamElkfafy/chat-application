...
import PrivataChatModal from "./_models/_profile/PrivataChatModal";
import PortalPrivateModal from "../lib/PortalPrivateModal";
export default function UserModal({
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const room = (await axios.get("rooms/" + response2.data.room)).data
          .payload;
        setRoom(room);
  ...
  const handleClickLike = async () => {
    try {
      const response = await axios.post(`users/checklikes`, {
        userFrom: user._id,
        userTo: userId,
      });
  ...
  const handleClickBlock = async () => {
    await axios.put(`users/updateblock/${user._id}`, {
      checkId: userId,
      check: resultBlock,
    });
  ...
  return (
    <>
      {/* {isPrivateChatOpen && (
        <PortalPrivateModal containerId={'portal-private-modal-root'}>
          <PrivataChatModal
          toUserId={userId}
          chatId={chatId}
      ...
      <Modal isOpen={isOpen} onClose={onClose}>
        ...
        >
          ...
          >
            <div className="flex">
              <img
                src={import.meta.env.VITE_API_BASE_URL + userData.img}
                className="w-6 h-6"
              />
              <p className="ps-2">{userData.name}</p>
            </div>
            <ModalCloseButton
              bg={getColor("closeButton")}
