...
import PrivataChatModal from "./PrivataChatModal";
...
function PrivateChatModule({ toUserId, onClose }: { toUserId: any, onClose: () => void}) {
  ...
  const handleClick = async () => {
    if (checkChat === true) {
      ...
      onClose();
      resetLists();
      eventEmitter.emit("updateData", {chatId: chat_id, toUserId: toUserId, isOpen: true})
      // setChatId(chat_id);
      socket.emit("join-room", chat_id);
      await axios.put(`users/updateprivate/${user._id}`, { userId: toUserId });
    }
  };
  return (
    <>
      ...
      {/* <Modal isOpen={isOpen} onClose={onClose}>
        <ModalContent>
          ...
          <div
            className="flex flex-col h-[300px] overflow-auto"
            ref={input}
